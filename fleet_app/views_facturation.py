...
@login_required
@user_owns_related_data(Facture, pk_url_kwarg='facture_pk', relation_path=['utilisateur'])
def ajouter_ligne_facture(request, facture_pk):
    """Ajouter une ligne à une facture existante
    Sécurisée pour garantir que l'utilisateur ne peut modifier que ses propres factures
    """
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            try:
                ...
                # Recalculer les montants de la facture
                facture.calculer_montants()
                messages.success(request, "Le produit a été ajouté à la facture avec succès.")
                return redirect('facture_detail', facture.numero)
            except Exception as e:
        ...
        form = LigneFactureForm(initial={'quantite': 1})
    
    context = {
        'form': form,
        'facture': facture,
        'titre': f'Ajouter un produit à la facture {facture.numero}',
    }
...
def modifier_ligne_facture(request, pk):
    """Modifier une ligne de facture existante"""
    # Vérifier que la ligne appartient à une facture de l'utilisateur connecté
    ligne = get_object_or_404(LigneFacture, pk=pk, facture__utilisateur=request.user)
    facture = ligne.facture
    ...
    context = {
        ...
        'ligne': ligne,
        'facture': facture,
        'titre': f'Modifier un produit de la facture {facture.numero}',
    }
...
def supprimer_ligne_facture(request, pk):
    ...
    if request.method == 'POST':
        ligne.delete()
        facture.calculer_montants()  # Recalculer les montants après suppression
    ...
    context = {
        'ligne': ligne,
        'facture': facture,
        'titre': f'Supprimer un produit de la facture {facture.numero}',
    }
    
    return render(request, 'fleet_app/inventaire/ligne_facture_confirm_delete.html', context)
