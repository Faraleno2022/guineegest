...
class CoutsFonctionnementFrame(ttk.Frame):
    ...
    def create_widgets(self):
        ...
        form_frame = ttk.LabelFrame(self, text="Enregistrement des coûts de fonctionnement")
        ...
        self.type_cout = ttk.Combobox(form_frame, values=["Carburant", "Entretien", "Réparation", "Assurance", "Taxe", "Autre"], width=17)
        ...
        ttk.Label(form_frame, text="Description:").grid(row=6, column=0, sticky="w", padx=5, pady=5)
        self.description = tk.Text(form_frame, width=40, height=3)
        self.description.grid(row=6, column=1, columnspan=2, padx=5, pady=5)
        ...
        # Tableau des coûts
        table_frame = ttk.LabelFrame(self, text="Liste des coûts de fonctionnement")
        ...
        self.couts_tree["columns"] = ("ID", "ID_Vehicule", "Date", "Type_Cout", "Montant", "Kilometrage", "Cout_Km")
    ...
    def load_couts(self):
        ...
        cursor = self.conn.cursor()
        cursor.execute('''
        SELECT ID, ID_Vehicule, Date, Type_Cout, Montant, Kilometrage, Cout_Par_Km
    ...
    def select_cout(self, event):
        ...
        cursor = self.conn.cursor()
        cursor.execute('''
        SELECT * FROM CoutsFonctionnement WHERE ID = ?
        ''', (values[0],))
    ...
    def add_cout(self):
        ...
        try:
            ...
            cursor = self.conn.cursor()
            cursor.execute('''
            INSERT INTO CoutsFonctionnement (
                ID_Vehicule, Date, Type_Cout, Montant, Kilometrage, Cout_Km, Description
            ) VALUES (?, ?, ?, ?, ?, ?, ?)
            ''', (
                self.id_vehicule.get(),
                self.date_cout.get(),
    ...
    def update_cout(self):
        ...
        if self.current_id:
            try:
                ...
                cursor = self.conn.cursor()
                cursor.execute('''
                UPDATE CoutsFonctionnement SET
                    ID_Vehicule = ?, Date = ?, Type_Cout = ?, Montant = ?, 
                    Kilometrage = ?, Cout_Km = ?, Description = ?
                WHERE ID = ?
                ''', (
                    self.id_vehicule.get(),
                    self.date_cout.get(),
                ...
                messagebox.showinfo("Succès", "Coût mis à jour avec succès")
                self.clear_form()
                self.load_couts()
    ...
    def delete_cout(self):
        # Vérifier si un coût est sélectionné
        if self.current_id:
            if messagebox.askyesno("Confirmation", "Êtes-vous sûr de vouloir supprimer ce coût ?"):
                try:
                    # Supprimer le coût
                    cursor = self.conn.cursor()
                    cursor.execute("DELETE FROM CoutsFonctionnement WHERE ID = ?", (self.current_id,))
                    self.conn.commit()
                    messagebox.showinfo("Succès", "Coût supprimé avec succès")
                    self.clear_form()
                    self.load_couts()
    ...
    def check_cout_alert(self, id_vehicule, type_cout, montant, kilometrage, cout_km):
        """Vérifie si le coût est anormal et crée une alerte si nécessaire"""
        if cout_km is None:
            return
        ...
        seuil = self.seuils_cout.get(type_vehicule, 0.15)  # Valeur par défaut si type inconnu
        
        if cout_km > seuil * 1.2:  # 20% au-dessus du seuil
            # Créer une alerte
            cursor.execute('''
            ...
            ''', (
                id_vehicule,
                'Coût excessif',
                f"Coût de fonctionnement élevé ({cout_km:.4f} €/km) pour {type_cout} le {self.date_cout.get()}",
            ...
            self.conn.commit()
            messagebox.warning("Alerte", f"Attention: Coût de fonctionnement élevé pour le véhicule {id_vehicule}!")
